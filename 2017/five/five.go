...
// DayFivePartOne of AoC 17
func DayFivePartOne() error {
	var d = make([]int, len(vals))
	copy(d, vals)
	maxD := len(d)
	idx, x := 0, 0
	...
	fmt.Printf("DayFivePartOne checksum is: %d, %d\n", x, idx)

	return nil
}

// DayFivePartTwo of AoC 17
func DayFivePartTwo() error {
	var d = make([]int, len(vals))
	copy(d, vals)
	maxD := len(d)
	idx, x := 0, 0
