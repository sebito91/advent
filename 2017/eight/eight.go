...
var re = regexp.MustCompile(`([a-z]+)\s+(.*)\s+([-,0-9]+)\s+if\s+([a-z]+)\s+(.*)\s+([-,0-9]+)`)

// Statement is the struct to house the operands, like:
// g dec 231 if bfx > -10
// Register, Option (enum), Value, Dependent, Condition, Evaluator
...
func helper(reg string, val, op int) {
	if op == INC {
		maps[reg] += val
		return
	}
...
func parseInput() []Statement {
	elems := make([]Statement, 1100)
	var str []string
	var c int
	...
	for _, x := range strings.Split(input, "\n") {
		...
		str = re.FindStringSubmatch(x)
		val, _ = strconv.Atoi(str[3])
		eval, _ = strconv.Atoi(str[6])
